# A Telegram HR Recruitment Bot with Google Drive Resume Upload (Secure via Railway Secrets + Error Handling)
...
def upload_to_drive(local_path, file_name):
    try:
        ...
        file_id = file.get('id')
        if not file_id:
            print("❌ Google Drive file creation failed, no file ID returned.")
            return None
        ...
        print("❌ Exception during Google Drive upload:", e)
        return None
